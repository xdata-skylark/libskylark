...
int run(const boost::mpi::communicator& comm, skylark::base::context_t& context,
    ...
    read(options.fileformat, options.trainfile, X, Y);
    ...
    if(!options.valfile.empty()) {
        ...
        read(options.fileformat, options.valfile, Xv, Yv, X.Height());
    }
    ...
    if(!options.testfile.empty()) {
        ...
        read(options.fileformat, options.testfile, Xt, Yt, X.Height());
