...
/**
 * Specialization for non distributed input Matrix and non distributed output Matrix
 */
template <typename ValueType>
struct FJLT_t <
    El::Matrix<ValueType>,
    El::Matrix<ValueType> > :
        ...
        virtual public sketch_transform_t<El::Matrix<ValueType>,
                                          El::Matrix<ValueType> > {
    // Typedef value, matrix, transform and transform data types
    // so that we can use them regularly and consistently.
    typedef ValueType value_type;
    typedef El::Matrix<value_type> matrix_type;
    typedef El::Matrix<value_type> output_matrix_type;

    typedef El::Matrix<ValueType> intermediate_type;
...
private:
    /**
     * Implementation for sketching Matrix -> Matrix and columnwise.
     */
    void apply_impl_dist(const matrix_type& A,
                    output_matrix_type& sketch_A,
                    skylark::sketch::columnwise_tag) const {
        ...
        // Create the sampled and scaled matrix
        intermediate_type dist_sketch_A(data_type::_S,
            inter_A.Width());
        double scale = sqrt((double)data_type::_N / (double)data_type::_S);
        for (int j = 0; j < inter_A.Width(); j++)
            for (int i = 0; i < data_type::_S; i++) {
                int row = data_type::samples[i];
                dist_sketch_A.Set(i, j,
                    scale * inter_A.Get(row, j));
            }
    ...
    /**
     * Implementation for sketching Matrix -> Matrix and rowwise.
     */
    void apply_impl_dist(const matrix_type& A,
                    output_matrix_type& sketch_of_A,
                    skylark::sketch::rowwise_tag) const {
