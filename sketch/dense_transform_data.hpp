...
struct dense_transform_data_t {
    ...
    dense_transform_data_t (int N, int S, skylark::sketch::context_t& context)
        : N(N), S(S), context(context),
          distribution(),
          random_samples(context.allocate_random_samples_array<value_type,
...
protected:
    ...
    const skylark::utility::random_samples_array_t <value_type,
                                   value_distribution_type> random_samples;
