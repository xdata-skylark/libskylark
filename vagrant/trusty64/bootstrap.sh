...
if [ ! -d "Elemental" ]; then
    ...
    mkdir build
    cd build
    cmake -DEL_USE_64BIT_INTS=ON -DEL_HAVE_QUADMATH=OFF -DHAVE-DCMAKE_BUILD_TYPE=Release -DEL_HYBRID=ON -DBUILD_SHARED_LIBS=ON -DMATH_LIBS="-L/usr/lib -llapack -lopenblas -lm" ../
